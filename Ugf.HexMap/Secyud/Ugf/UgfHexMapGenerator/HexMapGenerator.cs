...
namespace Secyud.Ugf.HexMap.Generator
{
	...
	public class HexMapGenerator :  IHexMapGenerator
	{
		...
		private void ErodeLand()
		{
			...
			{
				UgfCell cell = _grid.GetCell(i).Get<UgfCell>();
		...
		private void EvolveClimate(int i)
		{
			...
			for (HexDirection d = HexDirection.Ne; d <= HexDirection.Nw; d++)
			{
				...
				ClimateData neighborClimate = _nextClimate[neighbor.Cell.Index];
				...
				_nextClimate[neighbor.Cell.Index] = neighborClimate;
			}
		...
		private UgfCell GetRandomCell(MapRegion region)
		{
			...
			return  _grid.GetCell(x,z) .Get<UgfCell>();
		}
